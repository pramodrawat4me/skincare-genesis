...
export default function ContactUsPage() {
  const [contactpage, setContactpage] = useState<PageBlock[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${baseURL}/wp-json/wp/v2/pages/366`)
      .then((res) => res.json())
      .then((data) => {
        const blocks: PageBlock[] =
          (data?.acf?.page_blocks as PageBlock[]) || [];
        setContactpage(blocks);
        setLoading(false);
      });
  }, []);
    ...
    <>
      ...
      <main>
        {loading ? (
          ...
          contactpage.map((block, index) => renderBlock(block, index))
        )}
      </main>
      <Footer />
    </>
