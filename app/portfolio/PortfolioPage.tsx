...
export default function PortfolioPage() {
  const [portfoliopage, setPortfoliopage] = useState<PageBlock[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${baseURL}/wp-json/wp/v2/pages/359`)
      .then((res) => res.json())
      .then((data) => {
        const blocks: PageBlock[] =
          (data?.acf?.page_blocks as PageBlock[]) || [];
        setPortfoliopage(blocks);
        setLoading(false);
      });
  }, []);
    ...
    <>
      ...
      <main>
        {loading ? (
          ...
          portfoliopage.map((block, index) => renderBlock(block, index))
        )}
      </main>
      <Footer />
    </>
