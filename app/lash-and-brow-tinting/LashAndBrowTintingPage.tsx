...
export default function LashAndBrowTintingPage() {
  const [lashAndBrowTinting, setLashAndBrowTinting] = useState<PageBlock[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${baseURL}/wp-json/wp/v2/pages/357`)
      .then((res) => res.json())
      .then((data) => {
        const blocks: PageBlock[] =
          (data?.acf?.page_blocks as PageBlock[]) || [];
        setLashAndBrowTinting(blocks);
        setLoading(false);
      });
  }, []);
    ...
    <>
      ...
      <main>
        {loading ? (
          ...
          lashAndBrowTinting.map((block, index) => renderBlock(block, index))
        )}
      </main>
      <Footer />
    </>
